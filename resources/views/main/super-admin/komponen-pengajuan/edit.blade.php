...
@section('title', 'Edit Komponen Pengajuan')

@section('content')
    <div class="container p-2">
        <div class="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
            <h4 class="fw-bold">EDIT KOMPONEN PENGAJUAN</h4>
        </div>

        <div class="card shadow border-0 rounded-4">
            <div class="card-body">
                <form id="komponenForm" method="POST"
                    action="{{ route('superAdmin.komponen-pengajuan-update', $komponen->id_komponen_pengajuan) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_komponen') is-invalid @enderror"
                            id="nama_komponen" name="nama_komponen" placeholder="Masukkan nama komponen"
                    ...
                    <div class="mb-3">
                        ...
                        <select id="tipe" name="tipe" class="form-select @error('tipe') is-invalid @enderror"
                            ...
                            <option value="text" {{ old('tipe', $komponen->tipe) == 'text' ? 'selected' : '' }}>Text</option>
                            <option value="textarea" {{ old('tipe', $komponen->tipe) == 'textarea' ? 'selected' : '' }}>
                                Textarea</option>
                            <option value="select" {{ old('tipe', $komponen->tipe) == 'select' ? 'selected' : '' }}>Select
                            </option>
                            <option value="radio" {{ old('tipe', $komponen->tipe) == 'radio' ? 'selected' : '' }}>Radio
                            </option>
                            <option value="file" {{ old('tipe', $komponen->tipe) == 'file' ? 'selected' : '' }}>File</option>
                            <option value="date" {{ old('tipe', $komponen->tipe) == 'date' ? 'selected' : '' }}>Date</option>
                        </select>
                        @error('tipe')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    {{-- Opsi Pilihan --}}
                    @php
                        $opsiData = old('opsi', $komponen->opsi);
                        if (is_string($opsiData)) {
                            $opsiList = json_decode($opsiData, true) ?? [];
                            ...
                            @if(!empty($opsiList))
                                @foreach($opsiList as $index => $opsi)
                                    <div class="input-group mb-2">
                                        <input type="text" name="opsi[]" class="form-control" value="{{ $opsi }}"
                            ...
                            @else
                                <div class="input-group mb-2">
                                    <input type="text" name="opsi[]" class="form-control"
                                        placeholder="Masukkan opsi (contoh: Barang)">
                                    <button type="button" class="btn btn-outline-success" onclick="addOpsi()">
                                        <i class="bi bi-plus-circle"></i>
                                    </button>
                                </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="is_required" class="form-select @error('is_required') is-invalid @enderror">
                            <option value="" disabled>Pilih kewajiban</option>
                            <option value="1" {{ old('is_required', $komponen->is_required) == '1' ? 'selected' : '' }}>Wajib
                                diisi oleh user</option>
                            <option value="0" {{ old('is_required', $komponen->is_required) == '0' ? 'selected' : '' }}>
                                Opsional</option>
                        </select>
                        @error('is_required')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="is_active" class="form-select @error('is_active') is-invalid @enderror">
                            ...
                            <option value="1" {{ old('is_active', $komponen->is_active) == '1' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="0" {{ old('is_active', $komponen->is_active) == '0' ? 'selected' : '' }}>Nonaktif
                            </option>
                        </select>
                        @error('is_active')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        ...
                        <button type="submit" id="submitBtn" class="btn btn-primary">
                            <span class="btn-text"><i class="bi bi-save"></i> Perbarui Komponen</span>
    ...
    <script>
        ...
        function addOpsi() {
            ...
            // Pastikan hanya baris terakhir yang memiliki tombol tambah (+)
